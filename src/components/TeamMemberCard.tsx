...
export default function TeamMemberCard({ name, role, image, delay = 0 }: TeamMemberProps) {
  ...
  return (
    <motion.div
      ...
      className="rounded-[19px] overflow-hidden shadow-[5px_15px_20px_rgba(0,0,0,0.25)] border border-[#979797] flex flex-col items-center text-center h-[280px] sm:h-[380px] lg:h-[460px] w-full cursor-pointer bg-[#F83131]"
    >
      <div className="w-full h-[160px] sm:h-[240px] lg:h-[300px] flex items-center justify-center shrink-0 px-4 lg:px-10 overflow-hidden">
        <motion.div
          className="w-[100px] h-[100px] sm:w-[140px] sm:h-[140px] lg:w-[170px] lg:h-[170px] rounded-full overflow-hidden border-[3px] lg:border-4 border-white/30 shrink-0"
      ...
      <div className="px-3 lg:px-6 pb-4 lg:pb-6 w-full h-[120px] sm:h-[140px] lg:h-[160px] flex flex-col justify-center items-center shrink-0">
        ...
        <p className="font-roboto text-xs sm:text-sm lg:text-[20px] text-white mt-0.5 lg:mt-1" style={{ opacity: 0.63 }}>{role}</p>
      </div>
    </motion.div>
  );
}
